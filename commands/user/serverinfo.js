...
module.exports = {
    ...
    callback: async (client, bot, message, args, text) => {
        ...
        message.guild.members.fetch().then(fetchedMembers => {
            ...
            const botCount2 = message.guild.members.cache.filter((member) => member.user.bot).size;
            const serverinfo = new Discord.MessageEmbed()
                ...
                .addField("Creation Date", `<t:${Math.floor(message.channel.guild.createdAt.getTime() / 1000)}:D> (<t:${Math.floor(client.user.createdAt.getTime() / 1000)}:R>)`, true)
