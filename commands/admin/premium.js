    ...
    callback: async (client, bot, message, args, text) => {
        ...
        if (!hasToken) {
            return message.channel.send({
                content: "You don't have any premium tokens!"
            })
        }
        ...
        switch (args[0]) {
            case "redeem":
                if (guildSettings.premium === true) {
                    return message.channel.send({
                        content: "This guild already has premium enabled! "
                    })
                }
                ...
                const premiumEnabled = new MessageEmbed()
                    ...
                    .setFooter({
                        text: "Premium enabled by " + message.author.tag,
                        iconURL: message.author.displayAvatarURL({
                            dynamic: true
                        })
                    })
                message.channel.send({
                    embeds: [premiumEnabled]
                })
                break;
            case "revoke":
                if (guildSettings.premiumHolder !== message.author.id) {
                    return message.channel.send({
                        content: "You cannot revoke someone else's premium!"
                    })
                }
                if (guildSettings.premium === false) {
                    return message.channel.send({
                        content: "Does not have premium enabled! "
                    })
                }
                await Tokens.findOneAndUpdate({
                    userID: message.author.id,
            ...
            case "check":
                if (guildSettings.premium === false) {
                    return message.channel.send({
                        content: "This guild does not have premium enabled! "
                    })
                }
                const premiumStatus = new MessageEmbed()
                    .setAuthor({
                        name: message.guild.name + "'s Premium Status",
            ...
            case "bal":
                ...
                const hoeEmbed = new MessageEmbed()
                    .setAuthor({
                        name: `${message.author.tag}'s Balance`,
                        iconURL: message.author.displayAvatarURL({
                            dynamic: true
                        })
                    })
